    ...
    def query(self, context_info, query_text):
        ...
        if (context_info is None) or (context_info == "") or (len(context_info) == 0):
            ...
            text = "Please reply based on the context information below:\n".join(
                f"{index}. {text}" for index, text in enumerate(context_info))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--system_prompt",
                        default="You are a customer service agent for company 9. You need to reply to the customer based on the context information below.",
                        dest="system_prompt", type=str, help="GPT system prompt")
    parser.add_argument("--post_prompt", default="Please reply to the customer.", dest="post_prompt", type=str,
                        help="GPT post prompt")
    ...
    if use_openai:
        # test for openai
        gpt = GPT(model=args.model, temperature=args.temperature, system_prompt=args.system_prompt,
                  post_prompt=args.post_prompt, use_history=args.use_history)
    else:
        # test for azure
        gpt = GPT(
    ...
    gpt.set_system_prompt(
        "You are a customer service agent for Segway. You need to reply to the customer based on the context information below:")
