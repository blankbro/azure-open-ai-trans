...
def create_meta(category: str, content: str, title: Optional[str] = None, url: Optional[str] = None,
...
class MyPinecone():
    ...
    def upsert(self, namespace: str,
               items: Union[List[Tuple[str, List[float], Dict[str, Any]]], Tuple[str, List[float], Dict[str, Any]]]):
    ...
    def query(self,
        ...
        return self.index.query(namespace=namespace, top_k=top_k, vector=vector, id=id, filter=filter,
    ...
    def query_meta(self,
        ...
        return [{'score': result['score'], 'metadata': result['metadata']} for result in
                self.index.query(namespace=namespace, top_k=top_k, vector=vector, id=id, filter=filter,
                                 include_metadata=True)['matches'] if result['score'] > threshold]
...
if __name__ == "__main__":
    ...
    if False:
        ...
        metas = [create_meta("Glossary", item, url="https://www.segway.com.cn/", label=["Glossary"]) for item in
                 testcase]
